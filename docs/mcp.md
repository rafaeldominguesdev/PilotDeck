...
```bash
claude mcp add --transport http pilotdeck http://<your-host>/mcp \
...
"PilotDeck is the task board where agents claim and deliver cards (not Overclock the
...
`pilotdeck://project/<PREFIX>/context`. `resources/list` only advertises projects that
