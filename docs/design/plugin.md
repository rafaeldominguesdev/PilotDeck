# Official PilotDeck plugin design
...
`plugin/` is the single portable package. `plugin/PILOTDECK.md` is the only full
...
| Skill | `skills/pilotdeck/SKILL.md` | Same | Same | Same | Same |
...
| MCP | `claude mcp add`, user scope | `grok mcp add`, user scope | `~/.kimi-code/mcp.json` | `[mcp_servers.pilotdeck]` in `config.toml` | `mcp_config.json` in the plugin (`serverUrl` + `headers`) - its only channel |
...
- A `${PILOTDECK_URL}` placeholder is not expanded by any of these CLIs from the
  package itself. Kimi drops the server silently. Claude keeps it and reports
  `${PILOTDECK_URL} (HTTP) - Failed to connect`. Anyone installing straight from a
...
- Where install.sh *did* resolve the URL, it wrote the server into the package copy
  **and** into the CLI's own configuration, so each CLI ended up holding two servers
  named `pilotdeck`. A stale package copy could then shadow a working connection.
...
And it will not take the name over: if a CLI already holds an `pilotdeck` server
...
```
claude mcp add --transport http --scope user --header "Authorization: Bearer …" pilotdeck <url>
```

hands `pilotdeck` and `<url>` to `--header` and dies on `missing required argument
...
5. The claim guard records `.pilotdeck/claim.json` after `task_claim`, removes it
...
documented in the canonical `PILOTDECK.md` instead of being hidden by a nominal
hook file.
...
`<config root>/pilotdeck/antigravity/pilotdeck`, writes the credentials and the
resolved `PILOTDECK.md` path into that copy, installs it, and re-checks the
...
an explicit `[mcp_servers.pilotdeck]` block and merged global hooks.

`PILOTDECK_INSTALL_HOME` redirects the *managers*, not only the files (OCL-114).
...
`GEMINI_HOME` whenever `PILOTDECK_INSTALL_HOME` is set. They are overrides rather
...
the shebang so an exported `PILOTDECK_TOKEN` still wins and a saved copy is
...
`<config root>/pilotdeck/plugin-src` checkout instead of an ephemeral `gh repo
...
`claude plugin marketplace add rafaeldominguesdev/pilotdeck` followed by
`claude plugin install pilotdeck@pilotdeck` clones the repository, materializes
`plugins/cache/pilotdeck/pilotdeck/<version>`, and reports the full component
...
install.sh also asks `claude plugin list --json` for the enabled pilotdeck
entry's `installPath` and checks that `PILOTDECK.md` actually exists there;
...
Claude receives an `@` import inside `<!-- pilotdeck:start -->` and
`<!-- pilotdeck:end -->`. A CLI without native plugin support receives one AGENTS.md
...
fallback; `PILOTDECK_AGENTS_FALLBACK=1` enables that reference when an additional,
...
- `url` is validated as a real URL. A `${PILOTDECK_URL}` placeholder is **not** expanded
...
`<KIMI_CODE_HOME>/plugins/managed/pilotdeck` and register it in
...
4. Hooks start in the plugin directory and `workspacePaths` is empty in print
   ...
   `PILOTDECK_WORKSPACE` overrides it.
...
- Validate the Codex plugin and the PilotDeck skill with their official local tools.
