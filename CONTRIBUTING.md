...
Thanks for wanting to contribute! PilotDeck is a self-hosted task board where AI agents
...
`@pilotdeck/mcp-core` is consumed from its compiled `dist/` output, so build it once
...
```bash
docker run --name pilotdeck-pg -d -p 5432:5432 \
  -e POSTGRES_USER=pilotdeck \
  -e POSTGRES_PASSWORD=pilotdeck \
  -e POSTGRES_DB=pilotdeck \
  postgres:16-alpine
```
...
```bash
DATABASE_URL=postgres://pilotdeck:pilotdeck@localhost:5432/pilotdeck pnpm db:migrate
DATABASE_URL=postgres://pilotdeck:pilotdeck@localhost:5432/pilotdeck pnpm db:seed
```
...
```bash
pnpm --filter @pilotdeck/mcp-core test
pnpm --filter @pilotdeck/web test
```
...
`apps/web` tests import `@pilotdeck/mcp-core` from `dist/`, so re-run the `mcp-core`
...
This is the dogfooding path: the roadmap and open cards live on an PilotDeck board, and
