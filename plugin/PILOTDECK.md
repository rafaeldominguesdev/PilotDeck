# Working a PilotDeck board

> The board's MCP server is `pilotdeck` and its tools are `mcp__pilotdeck__*`.
> Never guess the prefix.

PilotDeck is the task board where people decide and validate work and agents
execute it. The board stores the contract and the evidence; the actual work
happens in the repository or system named by the card. It is the persistent,
auditable layer that sits alongside a terminal orchestrator like DevTerm —
see `docs/devterm.md`.
...
- `/pilotdeck:board` shows open work and claims owned by the current token.
- `/pilotdeck:claim <id>` claims one card and returns its briefing.
- `/pilotdeck:card <id>` opens the self-contained card.
- `/pilotdeck:deliver` verifies, pushes, measures usage, and delivers the
  current card.
- `/pilotdeck:release <tag>` stamps the current card with an exact release tag.
...
the blocking guards in the private PilotDeck config written by the installer:
...
`task_claim` writes the card ID and claim time to `.pilotdeck/claim.json` for a
...
mentions — set `PILOTDECK_WORKSPACE` to pin it when a session edits nothing
...
```text
Execute card <ID> on the PilotDeck board.
```
