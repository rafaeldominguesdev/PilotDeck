...
```bash
grok plugin marketplace add rafaeldominguesdev/pilotdeck
grok plugin install pilotdeck
```
...
```bash
grok plugin install 'rafaeldominguesdev/pilotdeck#plugin'
```
...
grok mcp add --transport http --scope user \
  ...
  pilotdeck https://<your-instance>/mcp
```

Verify with `grok plugin details pilotdeck` and `grok inspect` — the latter lists the
`pilotdeck` skill, the five slash commands, the hooks, and the MCP server.
...
|---|---|
| `skills/pilotdeck` | The workflow: how to read a card contract, claim it, deliver it with measured usage |
...
| `.mcp.json` | Empty on purpose: the package declares no MCP server. The installer writes the `pilotdeck` server into each CLI's own configuration, with your instance URL resolved, so a marketplace install can never leave you a server that cannot connect |
| `PILOTDECK.md` | The canonical rules the skill and commands both defer to |

## Network endpoints and credentials

The plugin talks to **exactly one endpoint: your own PilotDeck instance**, whose URL you
...
- **Credential:** a bearer token for your instance. The installer writes it to
  `${XDG_CONFIG_HOME:-~/.config}/pilotdeck/config` with mode `600`, or you pass it to your
...
waits for a claim. See `PILOTDECK.md` for the exact rule and its known limit.
