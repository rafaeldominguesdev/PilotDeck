...
# payload into the shape the shared PilotDeck hooks already understand, runs
...
oc_agy_normalize() {
  if command -v python3 >/dev/null 2>&1; then
    OC_AGY_WORKSPACE=${PILOTDECK_WORKSPACE:-} python3 -c '
import json, os, sys
...
'
  ...
  if command -v node >/dev/null 2>&1; then
    ...
    OC_AGY_WORKSPACE=${PILOTDECK_WORKSPACE:-} node -e '
    ...
    case "$tool_name" in
      ...
      task_deliver|mcp__*__task_deliver)
        ...
        if complaint=$(cd "${repo_root:-.}" && printf '%s' "$normalized" | node "$SCRIPT_DIR/post-deliver.mjs" 2>&1 >/dev/null); then
          ...
          [ -n "$complaint" ] || complaint="PilotDeck could not verify the delivered commit."
