...
failOpen(async () => {
  ...
  if (!taskType) {
    block("PilotDeck could not determine the task type for harness recommendation.");
    return;
  }

  const recommendation = await mcpCall("harness_recommend", JSON.stringify({ type: taskType }));
  if (!recommendation) {
    block("PilotDeck could not read the current harness recommendation. Retry before task_create.");
    return;
  }
