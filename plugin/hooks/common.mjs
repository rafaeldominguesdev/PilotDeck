// Shared, side-effect-free helpers for every PilotDeck hook.
//
...
export function configFile() {
  if (process.env.PILOTDECK_CONFIG_FILE) return process.env.PILOTDECK_CONFIG_FILE;
  const base = process.env.XDG_CONFIG_HOME || path.join(os.homedir(), ".config");
  return path.join(base, "pilotdeck", "config");
}
...
export async function mcpCall(tool, argumentsJson) {
  ...
  const body = `{"jsonrpc":"2.0","id":"pilotdeck-hook","method":"tools/call","params":{"name":${JSON.stringify(
    tool,
...
export function claimFile(root) {
  ...
  return path.join(base, ".pilotdeck", "claim.json");
}
