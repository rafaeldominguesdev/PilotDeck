...
if (!commit) {
  refuse("PilotDeck delivery evidence must cite the full Git commit ID.");
}

if (!git(["rev-parse", "--is-inside-work-tree"]).ok) {
  refuse("PilotDeck could not verify the delivered commit outside a Git worktree.");
}

if (!git(["cat-file", "-e", `${commit}^{commit}`]).ok) {
  refuse("The commit cited in PilotDeck delivery evidence is not present locally.");
}

if (!git(["fetch", "--all", "--quiet", "--prune"]).ok) {
  refuse("PilotDeck could not refresh remote refs to verify the delivered commit.");
}

const remotes = git(["for-each-ref", `--contains=${commit}`, "--format=%(refname)", "refs/remotes"], true);
if (!remotes.ok || !remotes.stdout.trim()) {
  refuse("The commit cited in PilotDeck delivery evidence is not present on a remote branch.");
}
