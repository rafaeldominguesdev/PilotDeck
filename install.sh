...
plugin_source="${PILOTDECK_PLUGIN_SOURCE:-rafaeldominguesdev/pilotdeck}"
install_home="${PILOTDECK_INSTALL_HOME:-$HOME}"
project_dir="${PILOTDECK_PROJECT_DIR:-$PWD}"

if [[ -n "${PILOTDECK_INSTALL_HOME:-}" ]]; then
  config_root="$install_home/.config"
else
  config_root="${XDG_CONFIG_HOME:-$install_home/.config}"
fi
pilotdeck_root="$config_root/pilotdeck"
plugin_target="$pilotdeck_root/plugin"
private_config="$pilotdeck_root/config"

# OCL-114: PILOTDECK_INSTALL_HOME only ever redirected the files this script
...
# `@/c/Users/.../PILOTDECK.md` line written into CLAUDE.md pointed at nothing:
...
if [[ -n "${PILOTDECK_INSTANCE_URL:-}" ]]; then
  instance_url=$PILOTDECK_INSTANCE_URL
else
  read -r -p "PilotDeck instance URL: " instance_url </dev/tty
fi
...
case "$instance_host" in
  invalid|test|example|localdomain|*.invalid|*.test|*.example|*.localdomain|example.com|example.net|example.org)
    printf '%s\n' "That instance URL is in a reserved namespace that can never resolve (RFC 2606/6761), so it is a test fixture rather than a board. Use the URL of your own PilotDeck instance." >&2
    exit 1
    ;;
esac
...
exchange_pairing_code() {
  ...
  if ! command -v curl >/dev/null 2>&1; then
    printf '%s\n' "curl is required to exchange an PilotDeck pairing code." >&2
    return 1
  fi
...
if [[ -n "${PILOTDECK_TOKEN:-}" ]]; then
  token=$PILOTDECK_TOKEN
elif [[ -n "${PILOTDECK_PAIRING_CODE:-}" ]]; then
  pairing_code=$PILOTDECK_PAIRING_CODE
  if [[ ! "$pairing_code" =~ ^[0-9]{6}$ ]]; then
    printf '%s\n' "An PilotDeck pairing code is exactly six digits." >&2
    exit 1
  fi
  ...
  printf '%s\n' "Paired with the PilotDeck instance. The token was not displayed."
else
  read -r -s -p "PilotDeck token: " token </dev/tty
  printf '\n' >/dev/tty
fi
...
plugin_src="$pilotdeck_root/plugin-src"
...
if [[ -f "$script_dir/plugin/PILOTDECK.md" ]]; then
  source_root=$script_dir
elif [[ -n "${PILOTDECK_PLUGIN_DIR:-}" && -f "$PILOTDECK_PLUGIN_DIR/PILOTDECK.md" ]]; then
  source_root=$(CDPATH='' cd -- "$PILOTDECK_PLUGIN_DIR/.." && pwd)
else
  ...
  # `claude plugin marketplace add rafaeldominguesdev/pilotdeck` directly.) The checkout
  ...
  if ! command -v git >/dev/null 2>&1; then
    printf '%s\n' "git is required to fetch the PilotDeck plugin package." >&2
    exit 1
  fi
  ...
  if [[ -d "$plugin_src/.git" ]]; then
    if ! git -C "$plugin_src" fetch --depth 1 origin >/dev/null 2>&1 || \
      ...
      printf '%s\n' "Could not update the local PilotDeck plugin checkout at $plugin_src." >&2
      exit 1
    fi
  else
    rm -rf -- "$plugin_src"
    if ! git clone --depth 1 "$clone_url" "$plugin_src" >/dev/null 2>&1; then
      printf '%s\n' "Could not clone the PilotDeck plugin package." >&2
      exit 1
    fi
  fi
...
if [[ ! -f "$source_root/plugin/.codex-plugin/plugin.json" || ! -f "$source_root/plugin/PILOTDECK.md" ]]; then
...
replace_marked_block() {
  ...
  mkdir -p "$directory"
  temporary=$(mktemp "$directory/.pilotdeck.XXXXXX")
  if [[ -f "$file" ]]; then
    awk '
      /<!-- pilotdeck:start -->/ { skip=1; next }
      /<!-- pilotdeck:end -->/ { skip=0; next }
      !skip { print }
    ' "$file" >"$temporary"
  fi
...
if mode == "mcp-kimi":
    ...
    data.setdefault("mcpServers", {})["pilotdeck"] = {
        "transport": "http",
    ...
    for event, rules in source.get("hooks", {}).items():
        ...
        for rule in hooks.get(event, []):
            ...
            if not any("pilotdeck" in command.lower() for command in commands):
                kept.append(rule)
        for rule in rules:
...
fd, temp_name = tempfile.mkstemp(prefix=".pilotdeck-", dir=path.parent)
...
if (process.env.OC_JSON_MODE === "mcp-kimi") {
  ...
  for (const [event, rules] of Object.entries(source.hooks ?? {})) {
    const kept = (data.hooks[event] ?? []).filter(rule =>
      !(rule.hooks ?? []).some(h => (h.command ?? "").toLowerCase().includes("pilotdeck"))
    );
...
const temp = file + ".pilotdeck-" + process.pid;
...
# OCL-114: the package used to declare its own `pilotdeck` MCP server on top of
...
# repository template carries an unexpanded ${PILOTDECK_URL}, the plugin-level
...
native_marketplace="$pilotdeck_root/native-marketplace"
...
if [[ -n "${PILOTDECK_CLIS:-}" ]]; then
  detected_clis=",${PILOTDECK_CLIS// /},"
else
  detected_clis=","
...
# A server already named `pilotdeck` in a CLI is somebody's board. Answering
...
for entry in plugins:
    if str(entry.get("id", "")).startswith("pilotdeck@") and entry.get("enabled"):
...
process.stdin.on("end", () => {
  ...
  const found = plugins.find(p => String(p.id || "").startsWith("pilotdeck@") && p.enabled);
...
for entry in data.get("installed", []):
    if entry.get("name") == "pilotdeck" and entry.get("installed") and entry.get("enabled"):
...
if has_cli claude; then
  if claude plugin marketplace add "$(native_path "$native_marketplace")" --scope user >/dev/null 2>&1 || \
    claude plugin marketplace update pilotdeck >/dev/null 2>&1; then
  ...
  claude plugin marketplace update pilotdeck >/dev/null 2>&1 || true
  if claude plugin install pilotdeck@pilotdeck --scope user >/dev/null 2>&1 || \
    claude plugin update pilotdeck@pilotdeck --scope user >/dev/null 2>&1; then
    printf '%s\n' "Claude plugin configured."
  else
    printf '%s\n' "Claude plugin needs manual confirmation in its native plugin manager." >&2
  fi
  claude plugin update pilotdeck@pilotdeck --scope user >/dev/null 2>&1 || true
  if foreign_mcp_registration claude mcp get pilotdeck; then
    printf '%s\n' "Claude already has an MCP server named pilotdeck pointing at a different instance; it was left untouched. Remove it with 'claude mcp remove pilotdeck' and re-run this installer to move to this board." >&2
  else
    claude mcp remove pilotdeck --scope user >/dev/null 2>&1 || true
    ...
    quiet_try "Claude MCP" claude mcp add --transport http --scope user \
      pilotdeck "$mcp_url" --header "Authorization: Bearer $token"
  fi
  claude_block=$(printf '%s\n' '<!-- pilotdeck:start -->' "@$plugin_target_native/PILOTDECK.md" '<!-- pilotdeck:end -->')
  ...
  if [[ -n "$claude_install_path" && -f "$claude_install_path/PILOTDECK.md" ]]; then
    assert_materialized_version "Claude plugin" "$claude_install_path" || validation_failed=1
  else
    printf '%s\n' "Claude plugin did not materialize: 'claude plugin list --json' shows no enabled pilotdeck entry with PILOTDECK.md on disk. Do not trust the earlier \"configured\" messages; retry or install manually." >&2
    validation_failed=1
  fi
fi

if has_cli codex; then
  codex_marketplace="$pilotdeck_root/codex-marketplace"
  ...
  # "./plugins/pilotdeck", so the payload has to live at <root>/plugins/pilotdeck;
  # copying it next to the manifest makes `codex plugin add` fail with
  # "plugin source path is not a directory".
  codex_plugin="$codex_marketplace/plugins/pilotdeck"
...
{
  "name": "pilotdeck",
  "interface": { "displayName": "PilotDeck" },
  "plugins": [
    {
      "name": "pilotdeck",
      "source": { "source": "local", "path": "./plugins/pilotdeck" },
...
JSON
  ...
  quiet_try "Codex plugin" codex plugin add pilotdeck@pilotdeck
  ...
  codex plugin marketplace update pilotdeck >/dev/null 2>&1 || true
  codex plugin update pilotdeck@pilotdeck >/dev/null 2>&1 || true
  write_codex_mcp

  codex_install_path=$(verify_codex_plugin) || codex_install_path=""
  if [[ -n "$codex_install_path" && -f "$codex_install_path/PILOTDECK.md" ]]; then
    assert_materialized_version "Codex plugin" "$codex_install_path" || validation_failed=1
  else
    printf '%s\n' "Codex plugin did not materialize: 'codex plugin list --json' shows no enabled pilotdeck entry with PILOTDECK.md on disk. Do not trust the earlier \"configured\" messages; retry or install manually." >&2
    validation_failed=1
  fi
...
if has_cli grok; then
  if grok plugin marketplace add "$(native_path "$native_marketplace")" >/dev/null 2>&1 || \
    grok plugin marketplace update pilotdeck >/dev/null 2>&1; then
  ...
  grok plugin marketplace update pilotdeck >/dev/null 2>&1 || true
  if grok plugin install "$plugin_target_native" --trust >/dev/null 2>&1 || \
    grok plugin update pilotdeck >/dev/null 2>&1; then
    printf '%s\n' "Grok plugin configured."
  else
    printf '%s\n' "Grok plugin needs manual confirmation in its native plugin manager." >&2
  fi
  grok plugin update pilotdeck >/dev/null 2>&1 || true
  if foreign_mcp_registration grok mcp get pilotdeck; then
    printf '%s\n' "Grok already has an MCP server named pilotdeck pointing at a different instance; it was left untouched. Remove it with 'grok mcp remove pilotdeck' and re-run this installer to move to this board." >&2
  else
...
kimi_register_plugin() {
  ...
  local managed="$kimi_home/plugins/managed/pilotdeck"
...
previous = next((p for p in data["plugins"] if p.get("id") == "pilotdeck"), {})
plugins = [p for p in data["plugins"] if p.get("id") != "pilotdeck"]
plugins.append({
    "id": "pilotdeck",
...
if has_cli agy; then
  ...
  # generic template.
  agy_stage="$pilotdeck_root/antigravity/pilotdeck"
  agy_installed="$install_home/.gemini/config/plugins/pilotdeck"
  ...
  agy_rule_block=$(printf '%s\n' '<!-- pilotdeck:start -->' \
    "Read and follow $(native_path "$agy_installed")/PILOTDECK.md for all PilotDeck board work." \
    '<!-- pilotdeck:end -->')
  ...
  "$agy_bin" plugin enable pilotdeck >/dev/null 2>&1 || true
  ...
  if "$agy_bin" plugin list 2>/dev/null | grep -q '"pilotdeck"' && \
    [[ -f "$agy_installed/PILOTDECK.md" ]]; then
    assert_materialized_version "Antigravity plugin" "$agy_installed" || validation_failed=1
  else
    printf '%s\n' "Antigravity plugin did not materialize: 'agy plugin list' shows no pilotdeck entry with PILOTDECK.md on disk. Retry or install manually." >&2
    validation_failed=1
  fi
fi

if [[ "${PILOTDECK_AGENTS_FALLBACK:-0}" = "1" ]] || \
  { ! has_cli claude && ! has_cli codex && ! has_cli grok && ! has_cli kimi && ! has_cli agy; }; then
  agents_block=$(printf '%s\n' '<!-- pilotdeck:start -->' "Read and follow $plugin_target_native/PILOTDECK.md for all PilotDeck board work." '<!-- pilotdeck:end -->')
...
if [[ "$validation_failed" = "1" ]]; then
  printf '%s\n' "PilotDeck plugin installation finished with unverified components; see warnings above. Credentials were stored privately and were not displayed." >&2
  exit 1
fi

printf '%s\n' "PilotDeck plugin installation complete. Credentials were stored privately and were not displayed."
