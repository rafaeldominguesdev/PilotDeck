...
# OCL-114. The package ships NO MCP server. A `${PILOTDECK_URL}` placeholder is
# never expanded by the CLIs, so every marketplace install used to hand the user
# an `pilotdeck` server that could not connect - and, on a machine where a run
...
if grep -rq 'PILOTDECK_URL' "$REPO_ROOT/plugin"; then
  echo "the package still carries an unexpandable PILOTDECK_URL placeholder" >&2
  exit 1
fi
...
jq -e '(.skills | index("./plugin/skills/pilotdeck")) and .commands == "./plugin/commands"
  and (.hooks | length == 6)
...
stage_cache() {
  ...
  touch "$1/PILOTDECK.md"
  printf '{"name":"pilotdeck","version":"%s","source":"./plugin"}\n' "$2" \
...
if [ "${1:-}" = "mcp" ] && [ "${2:-}" = "get" ]; then
  [ -n "${OC_TEST_EXISTING_MCP:-}" ] && printf 'pilotdeck: %s (HTTP)\n' "$OC_TEST_EXISTING_MCP"
fi
if [ "${1:-}" = "plugin" ] && [ "${2:-}" = "list" ]; then
  case "$(basename -- "$0")" in
    claude)
      printf '[{"id":"pilotdeck@pilotdeck","enabled":true,"installPath":"%s"}]' "$OC_TEST_CLAUDE_CACHE"
      ;;
    ...
    codex)
      printf '{"installed":[{"name":"pilotdeck","installed":true,"enabled":true,"source":{"path":"%s"}}]}' "$OC_TEST_CODEX_CACHE"
      ;;
  esac
fi
exit 0
...
plugins="$PILOTDECK_INSTALL_HOME/.gemini/config/plugins"
case "${1:-}:${2:-}" in
  plugin:install)
    mkdir -p "$plugins/pilotdeck"
    cp -R "$3/." "$plugins/pilotdeck/"
    ;;
  plugin:list)
    [ -d "$plugins/pilotdeck" ] && printf '{"imports":[{"name":"pilotdeck","source":"antigravity"}]}'
    ;;
esac
exit 0
SH
...
run_installer() {
  ...
  PILOTDECK_INSTALL_HOME="$TEST_ROOT/home" \
  PILOTDECK_PROJECT_DIR="$TEST_ROOT/project" \
  PILOTDECK_INSTANCE_URL="$FIXTURE_URL" \
  PILOTDECK_TOKEN="fixture" \
  PILOTDECK_CLIS="claude,codex,grok,kimi,agy" \
...
test "$(grep -c '<!-- pilotdeck:start -->' "$TEST_ROOT/home/.claude/CLAUDE.md")" -eq 1
test "$(grep -c '^# pilotdeck:start$' "$TEST_ROOT/home/.codex/config.toml")" -eq 1
...
test "$(grep -c '^enforce_claim=0$' "$TEST_ROOT/home/.config/pilotdeck/config")" -eq 1
test "$(grep -c '^token=' "$TEST_ROOT/home/.config/pilotdeck/config")" -eq 1
test "$(file_mode "$TEST_ROOT/home/.config/pilotdeck/config")" -eq 600
...
test "$(jq -r '[.plugins[] | select(.id == "pilotdeck")] | length' \
  "$TEST_ROOT/home/.kimi-code/plugins/installed.json")" -eq 1
jq -e 'has("mcpServers") | not' \
  "$TEST_ROOT/home/.kimi-code/plugins/managed/pilotdeck/kimi.plugin.json" >/dev/null
...
jq -e --arg url "$FIXTURE_URL/mcp" '.mcpServers.pilotdeck.transport == "http"
  and .mcpServers.pilotdeck.url == $url
  and (.mcpServers.pilotdeck | has("type") | not)' \
  "$TEST_ROOT/home/.kimi-code/mcp.json" >/dev/null
test -f "$TEST_ROOT/home/.kimi-code/plugins/managed/pilotdeck/hooks/claim-guard.mjs"
...
# template, the rule pointing at the PILOTDECK.md that exists on disk, and a
# second copy of the credentials that is no more readable than the first.
agy_plugin="$TEST_ROOT/home/.gemini/config/plugins/pilotdeck"
jq -e '(.mcpServers.pilotdeck.serverUrl | startswith("$") | not)
  and .mcpServers.pilotdeck.headers.Authorization
  and (.mcpServers.pilotdeck | has("type") | not)' \
  "$agy_plugin/mcp_config.json" >/dev/null
test "$(file_mode "$agy_plugin/mcp_config.json")" -eq 600
test "$(grep -c '<!-- pilotdeck:start -->' "$agy_plugin/rules/AGENTS.md")" -eq 1
grep -Fq "$agy_plugin/PILOTDECK.md" "$agy_plugin/rules/AGENTS.md"
test -f "$agy_plugin/PILOTDECK.md"
test -x "$agy_plugin/hooks/antigravity.sh"
jq -e '.pilotdeck | (.PreToolUse | length == 2) and (.PostToolUse | length == 1)
...
# CLI can end up with two `pilotdeck` servers, and no dead one can survive in a
# marketplace directory after the instance it was baked from is gone.
for copy in \
  "$TEST_ROOT/home/.config/pilotdeck/plugin" \
  "$TEST_ROOT/home/.config/pilotdeck/native-marketplace/plugin" \
  "$TEST_ROOT/home/.config/pilotdeck/codex-marketplace/plugins/pilotdeck"; do
...
# PILOTDECK_INSTALL_HOME (OCL-114).
...
sed -i.bak 's/enforce_claim=0/enforce_claim=1/' "$TEST_ROOT/home/.config/pilotdeck/config"
run_installer
test "$(grep -c '^enforce_claim=1$' "$TEST_ROOT/home/.config/pilotdeck/config")" -eq 1
...
if PATH="$TEST_ROOT/bin:$PATH" \
  ...
  OC_TEST_CODEX_CACHE="$TEST_ROOT/codex-cache" \
  PILOTDECK_INSTALL_HOME="$TEST_ROOT/home-unverified" \
  PILOTDECK_PROJECT_DIR="$TEST_ROOT/project" \
  PILOTDECK_INSTANCE_URL="$FIXTURE_URL" \
  PILOTDECK_TOKEN="fixture" \
  PILOTDECK_CLIS="claude" \
...
if PATH="$TEST_ROOT/bin:$PATH" \
  ...
  OC_TEST_CODEX_CACHE="$TEST_ROOT/codex-cache" \
  PILOTDECK_INSTALL_HOME="$TEST_ROOT/home-stale" \
  PILOTDECK_PROJECT_DIR="$TEST_ROOT/project" \
  PILOTDECK_INSTANCE_URL="$FIXTURE_URL" \
  PILOTDECK_TOKEN="fixture" \
  PILOTDECK_CLIS="claude" \
...
printf 'package v1\n' >"$TEST_ROOT/plugin-remote/plugin/PILOTDECK.md"
...
printf '{"name":"pilotdeck","version":"9.9.9"}' \
...
run_clone_installer() {
  ...
  OC_TEST_CODEX_CACHE="$TEST_ROOT/codex-cache" \
  PILOTDECK_INSTALL_HOME="$TEST_ROOT/clone-home" \
  PILOTDECK_PROJECT_DIR="$TEST_ROOT/project" \
  PILOTDECK_INSTANCE_URL="$FIXTURE_URL" \
  PILOTDECK_TOKEN="fixture" \
  PILOTDECK_CLIS="claude" \
  PILOTDECK_PLUGIN_SOURCE="file://$TEST_ROOT/plugin-remote" \
    "$TEST_ROOT/isolated-installer/install.sh" >"$TEST_ROOT/clone.out" 2>"$TEST_ROOT/clone.err"
}

run_clone_installer
plugin_src="$TEST_ROOT/clone-home/.config/pilotdeck/plugin-src"
test -d "$plugin_src/.git"
grep -Fq 'package v1' "$plugin_src/plugin/PILOTDECK.md"
...
run_clone_installer
grep -Fq 'package v2' "$plugin_src/plugin/PILOTDECK.md"

PILOTDECK_INSTALL_HOME="$TEST_ROOT/home-fallback" \
PILOTDECK_PROJECT_DIR="$TEST_ROOT/project" \
PILOTDECK_INSTANCE_URL="$FIXTURE_URL" \
PILOTDECK_TOKEN="fixture" \
PILOTDECK_CLIS="other" \
...
PILOTDECK_CLIS="other" \
  "$REPO_ROOT/install.sh" >"$TEST_ROOT/fallback.out" 2>"$TEST_ROOT/fallback.err"
test "$(grep -c '<!-- pilotdeck:start -->' "$TEST_ROOT/project/AGENTS.md")" -eq 1
...
run_pair_installer() {
  ...
  PILOTDECK_INSTALL_HOME="$TEST_ROOT/home-$1" \
  PILOTDECK_PROJECT_DIR="$TEST_ROOT/project" \
  PILOTDECK_INSTANCE_URL="$FIXTURE_URL" \
  PILOTDECK_TOKEN="" \
  PILOTDECK_PAIRING_CODE="$2" \
  PILOTDECK_CLIS="other" \
...
grep -Fq 'token=paired-secret' "$TEST_ROOT/home-pair/.config/pilotdeck/config"
...
test ! -f "$TEST_ROOT/home-pair-refused/.config/pilotdeck/config"
...
snapshot=$(PATH="$HOOK_PATH" PILOTDECK_CONFIG_FILE="$HOOK_CONFIG" node "$REPO_ROOT/plugin/hooks/session-start.mjs")
printf '%s' "$snapshot" | grep -q 'T-1'
printf '%s' "$snapshot" | grep -q 'PilotDeck board snapshot'
...
token=wrong
EOF
test -z "$(PATH="$HOOK_PATH" PILOTDECK_CONFIG_FILE="$TEST_ROOT/hook-config-bad-token" node "$REPO_ROOT/plugin/hooks/session-start.mjs")"

test -z "$(PATH="$HOOK_PATH" PILOTDECK_CONFIG_FILE="$HOOK_CONFIG" node "$REPO_ROOT/plugin/hooks/stop-guard.mjs")"
sed -i.bak 's/enforce_stop=0/enforce_stop=1/' "$HOOK_CONFIG"
stop_result=$(PATH="$HOOK_PATH" PILOTDECK_CONFIG_FILE="$HOOK_CONFIG" node "$REPO_ROOT/plugin/hooks/stop-guard.mjs")
...
test -z "$(printf '%s' "$matching_input" | PATH="$HOOK_PATH" PILOTDECK_CONFIG_FILE="$HOOK_CONFIG" node "$REPO_ROOT/plugin/hooks/pre-create.mjs")"
mismatched_input='{"tool_input":{"type":"feature","harness":{"cli":"codex","model":"other-model","effort":"high"}}}'
pre_result=$(printf '%s' "$mismatched_input" | PATH="$HOOK_PATH" PILOTDECK_CONFIG_FILE="$HOOK_CONFIG" node "$REPO_ROOT/plugin/hooks/pre-create.mjs")
...
test -z "$(printf '%s' "$write_input" | PATH="$HOOK_PATH" PILOTDECK_CONFIG_FILE="$HOOK_CONFIG" node "$REPO_ROOT/plugin/hooks/claim-guard.mjs")"

sed -i.bak 's/enforce_claim=0/enforce_claim=1/' "$HOOK_CONFIG"
blocked_write=$(printf '%s' "$write_input" | PATH="$HOOK_PATH" PILOTDECK_CONFIG_FILE="$HOOK_CONFIG" node "$REPO_ROOT/plugin/hooks/claim-guard.mjs")
...
test -z "$(printf '%s' "$read_input" | PATH="$HOOK_PATH" PILOTDECK_CONFIG_FILE="$HOOK_CONFIG" node "$REPO_ROOT/plugin/hooks/claim-guard.mjs")"
...
blocked_bash=$(printf '%s' "$write_bash_input" | PATH="$HOOK_PATH" PILOTDECK_CONFIG_FILE="$HOOK_CONFIG" node "$REPO_ROOT/plugin/hooks/claim-guard.mjs")
...
blocked_commit=$(printf '%s' "$commit_bash_input" | PATH="$HOOK_PATH" PILOTDECK_CONFIG_FILE="$HOOK_CONFIG" node "$REPO_ROOT/plugin/hooks/claim-guard.mjs")
...
guard_shell() {
  jq -nc --arg cwd "$TEST_ROOT/project" --arg tool "$1" --arg command "$2" \
    ...
    PATH="$HOOK_PATH" PILOTDECK_CONFIG_FILE="$HOOK_CONFIG" node "$REPO_ROOT/plugin/hooks/claim-guard.mjs"
}
guard_blocks() {
...
printf '%s' "$shellless_input" | PATH="$HOOK_PATH" PILOTDECK_CONFIG_FILE="$HOOK_CONFIG" node "$REPO_ROOT/plugin/hooks/claim-guard.mjs" |
...
for quiet_tool in Read Grep Glob TodoWrite WebFetch mcp__pilotdeck__task_list mcp__acme__list_widgets; do
  ...
  if [ -n "$(printf '%s' "$quiet_input" | PATH="$HOOK_PATH" PILOTDECK_CONFIG_FILE="$HOOK_CONFIG" node "$REPO_ROOT/plugin/hooks/claim-guard.mjs")" ]; then
...
printf '%s' "$unknown_write_input" | PATH="$HOOK_PATH" PILOTDECK_CONFIG_FILE="$HOOK_CONFIG" node "$REPO_ROOT/plugin/hooks/claim-guard.mjs" |
  grep -Fq 'claima um card no board antes: task_claim {id}'
...
agy_guard() {
  (cd "$agy_plugin" && printf '%s' "$1" |
    PILOTDECK_CONFIG_FILE="$HOOK_CONFIG" sh ./hooks/antigravity.sh pre-tool)
}
...
printf '%s' "$failed_claim_input" | PATH="$HOOK_PATH" PILOTDECK_CONFIG_FILE="$HOOK_CONFIG" node "$REPO_ROOT/plugin/hooks/claim-guard.mjs"
test ! -e "$TEST_ROOT/project/.pilotdeck/claim.json"

claim_input=$(jq -nc --arg cwd "$TEST_ROOT/project" '{cwd:$cwd,session_id:"session-fixture",tool_name:"mcp__pilotdeck__task_claim",tool_input:{task_id:"T-1"},tool_response:{structuredContent:{task:{short_id:"T-1",status:"em_execucao"},attempt:{id:"attempt-fixture",started_at:"2026-08-19T12:00:00.000Z"}}}}')
printf '%s' "$claim_input" | PATH="$HOOK_PATH" PILOTDECK_CONFIG_FILE="$HOOK_CONFIG" node "$REPO_ROOT/plugin/hooks/claim-guard.mjs"
test -f "$TEST_ROOT/project/.pilotdeck/claim.json"
test "$(file_mode "$TEST_ROOT/project/.pilotdeck/claim.json")" -eq 600
jq -e '.task_id == "T-1" and .claimed_at == "2026-08-19T12:00:00.000Z" and .session_id == "session-fixture"' "$TEST_ROOT/project/.pilotdeck/claim.json" >/dev/null
test -z "$(printf '%s' "$write_input" | PATH="$HOOK_PATH" PILOTDECK_CONFIG_FILE="$HOOK_CONFIG" node "$REPO_ROOT/plugin/hooks/claim-guard.mjs")"
...
blocked_other_session=$(printf '%s' "$other_session_input" | PATH="$HOOK_PATH" PILOTDECK_CONFIG_FILE="$HOOK_CONFIG" node "$REPO_ROOT/plugin/hooks/claim-guard.mjs")
printf '%s' "$blocked_other_session" | grep -Fq 'claima um card no board antes: task_claim {id}'

deliver_input=$(jq -nc --arg cwd "$TEST_ROOT/project" '{cwd:$cwd,session_id:"session-fixture",tool_name:"mcp__pilotdeck__task_deliver",tool_input:{task_id:"T-1"}}')
printf '%s' "$deliver_input" | PATH="$HOOK_PATH" PILOTDECK_CONFIG_FILE="$HOOK_CONFIG" node "$REPO_ROOT/plugin/hooks/claim-guard.mjs"
test ! -e "$TEST_ROOT/project/.pilotdeck/claim.json"
blocked_after_deliver=$(printf '%s' "$write_input" | PATH="$HOOK_PATH" PILOTDECK_CONFIG_FILE="$HOOK_CONFIG" node "$REPO_ROOT/plugin/hooks/claim-guard.mjs")
...
printf '%s' "$release_input" | PATH="$HOOK_PATH" PILOTDECK_CONFIG_FILE="$HOOK_CONFIG" node "$REPO_ROOT/plugin/hooks/claim-guard.mjs"
test ! -e "$TEST_ROOT/project/.pilotdeck/claim.json"
...
printf '%s' "$bare_claim_input" | PATH= PILOTDECK_CONFIG_FILE="$HOOK_CONFIG" "$NODE_BIN" "$REPO_ROOT/plugin/hooks/claim-guard.mjs"
test -f "$TEST_ROOT/project-bare/.pilotdeck/claim.json"
...
test -z "$(printf '%s' "$bare_write_input" | PATH= PILOTDECK_CONFIG_FILE="$HOOK_CONFIG" "$NODE_BIN" "$REPO_ROOT/plugin/hooks/claim-guard.mjs")"
...
printf '%s' "$bare_deliver_input" | PATH= PILOTDECK_CONFIG_FILE="$HOOK_CONFIG" "$NODE_BIN" "$REPO_ROOT/plugin/hooks/claim-guard.mjs"
test ! -e "$TEST_ROOT/project-bare/.pilotdeck/claim.json"
has_claim 1
bare_snapshot=$(PATH= PILOTDECK_CONFIG_FILE="$HOOK_CONFIG" "$NODE_BIN" "$REPO_ROOT/plugin/hooks/session-start.mjs")
...
if PATH="$TEST_ROOT/bin:$PATH" \
  ...
  OC_TEST_CLAUDE_CACHE="$TEST_ROOT/claude-cache" \
  OC_TEST_CODEX_CACHE="$TEST_ROOT/codex-cache" \
  PILOTDECK_INSTALL_HOME="$TEST_ROOT/home-reserved" \
  PILOTDECK_PROJECT_DIR="$TEST_ROOT/project" \
  PILOTDECK_INSTANCE_URL="$(printf '%s%s%s' 'https' '://' 'board.invalid')" \
  PILOTDECK_TOKEN="fixture" \
  PILOTDECK_CLIS="claude" \
...
test ! -e "$TEST_ROOT/home-reserved/.config/pilotdeck/config"
test ! -e "$TEST_ROOT/reserved-native.log"

# A server already named `pilotdeck` and pointing somewhere else is somebody's
...
PILOTDECK_INSTALL_HOME="$TEST_ROOT/home-collision" \
PILOTDECK_PROJECT_DIR="$TEST_ROOT/project" \
PILOTDECK_INSTANCE_URL="$FIXTURE_URL" \
PILOTDECK_TOKEN="fixture" \
PILOTDECK_CLIS="claude" \
...
PILOTDECK_INSTALL_HOME="$TEST_ROOT/home-refresh" \
PILOTDECK_PROJECT_DIR="$TEST_ROOT/project" \
PILOTDECK_INSTANCE_URL="$FIXTURE_URL" \
PILOTDECK_TOKEN="fixture" \
PILOTDECK_CLIS="claude" \
...
if ! PATH="$TEST_ROOT/bin:$TEST_ROOT/nochmod" \
  ...
  OC_TEST_CLAUDE_CACHE="$TEST_ROOT/claude-cache" \
  OC_TEST_CODEX_CACHE="$TEST_ROOT/codex-cache" \
  PILOTDECK_INSTALL_HOME="$TEST_ROOT/home-nochmod" \
  PILOTDECK_PROJECT_DIR="$TEST_ROOT/project" \
  PILOTDECK_INSTANCE_URL="$FIXTURE_URL" \
  PILOTDECK_TOKEN="fixture" \
  PILOTDECK_CLIS="claude,codex,grok,kimi,agy" \
...
test "$(grep -c '^token=' "$TEST_ROOT/home-nochmod/.config/pilotdeck/config")" -eq 1
...
# `@/c/Users/.../PILOTDECK.md` line it wrote into CLAUDE.md pointed at nothing,
...
test "$(native_path_of "$TEST_ROOT/winbin" '/c/Users/LASCHUK/.config/pilotdeck/plugin')" \
  = 'C:/Users/LASCHUK/.config/pilotdeck/plugin'
...
stale_copies="$TEST_ROOT/home/.config/pilotdeck/plugin
$TEST_ROOT/home/.config/pilotdeck/native-marketplace/plugin
$TEST_ROOT/home/.config/pilotdeck/codex-marketplace/plugins/pilotdeck
$TEST_ROOT/home/.config/pilotdeck/antigravity/pilotdeck"
