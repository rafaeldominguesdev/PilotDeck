...
function assertRealProductionDb(): number {
  ...
  if (!psList.includes("pilotdeck-app-1") || !psList.includes(CONTAINER)) {
    throw new Error(
      `Expected containers pilotdeck-app-1 and ${CONTAINER} both running on ${maskHost(SSH_HOST)}; got: ${psList.trim()}. Stopping instead of writing to a database nothing serves.`,
    );
  }
