...
const CONTAINER = "pilotdeck-pilotdeck-db-1";
const DB_USER = "pilotdeck";
const DB_NAME = "pilotdeck";
...
function fetchTopOfReport(): { attemptCount: number; wiredCheck: string; header: string } {
  ...
  const wiresDbByName = compose.includes("pilotdeck-db:5432/pilotdeck") && compose.includes("pilotdeck-db:");
  if (!wiresDbByName) {
    throw new Error(
      "deploy/docker-compose.cloud.yml no longer wires app -> pilotdeck-db/pilotdeck the way this audit assumes. Stopping instead of guessing which database /insights reads.",
    );
  }
  ...
  const appUp = psList.includes("pilotdeck-app-1");
  const dbUp = psList.includes(CONTAINER);
  if (!appUp || !dbUp) {
    throw new Error(
      `Expected containers pilotdeck-app-1 and ${CONTAINER} both running on ${SSH_HOST}; got: ${psList.trim()}. Stopping instead of reading a database nothing serves.`,
    );
  }
  ...
  const header =
    ...
    `Confirmado como o banco de /insights por dois fatos, não por suposição: (1) \`deploy/docker-compose.cloud.yml\` liga a env \`DATABASE_URL\` do serviço \`app\` a \`postgres://pilotdeck:***@pilotdeck-db:5432/pilotdeck\`, o mesmo serviço; (2) \`docker ps\` no host mostra \`pilotdeck-app-1\` e \`${CONTAINER}\` rodando ao mesmo tempo, no mesmo compose project. \`execution_attempt\` tem **${attemptCount}** linhas agora.`;
 ...
 * via PILOTDECK_CLAIMED_AT; this only closes the end so a shared transcript
    ...
    try {
      ...
      const stdout = execFileSync("node", [scriptFile], {
        ...
        env: {
          ...
          PILOTDECK_CLAIMED_AT: attempt.started_at,
