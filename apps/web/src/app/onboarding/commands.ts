 ...
 * PowerShell. It is also idempotent: it rewrites the `# pilotdeck:` block
...
const codexScript = (baseUrl: string, secret: string) => [
  ...
  "const t=line.trim();",
  "if(t==='# pilotdeck:start'){skip=true;return false}",
  "if(t==='# pilotdeck:end'){skip=false;return false}",
  "return skip===false})}catch(e){}",
  "while(kept.length>0&&kept[kept.length-1].trim()===''){kept.pop()}",
  "if(kept.length>0){kept.push('')}",
  "kept.push('# pilotdeck:start','[mcp_servers.pilotdeck]',",
  `'url = '+q+'${baseUrl}'+q,`,
  ...
  "console.log('pilotdeck configured in '+file)",
].join("");
  ...
  switch (cli) {
    case "claude-code":
      return assertCrossShell(
        `claude mcp add --transport http pilotdeck ${baseUrl} ${header}`,
      );
    case "codex":
      return assertCrossShell(`node -e "${codexScript(baseUrl, secret)}"`);
    case "gemini-cli":
      return assertCrossShell(
        `gemini mcp add --transport http pilotdeck ${baseUrl} ${header}`,
      );
    default:
