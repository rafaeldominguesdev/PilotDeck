...
describe("commandFor(claude-code) / commandFor(gemini-cli)", () => {
  it("adds the remote server in one `mcp add` call with the auth header", () => {
    expect(commandFor("claude-code", URL, SECRET)).toBe(
      `claude mcp add --transport http pilotdeck ${URL} --header "Authorization: Bearer ${SECRET}"`,
    );
    expect(commandFor("gemini-cli", URL, SECRET)).toBe(
      `gemini mcp add --transport http pilotdeck ${URL} --header "Authorization: Bearer ${SECRET}"`,
    );
  });
});
...
describe("commandFor(codex)", () => {
  ...
  it("writes the pilotdeck server into CODEX_HOME/config.toml", () => {
    const written = run(mkdtempSync(join(tmpdir(), "codex-")));

    expect(written).toContain("[mcp_servers.pilotdeck]");
  ...
  it("replaces its own block instead of appending a second one", () => {
    ...
    expect(written.split("[mcp_servers.pilotdeck]").length - 1).toBe(1);
    expect(written.split("# pilotdeck:start").length - 1).toBe(1);
  });
});
