...
import { project, workspace } from "@pilotdeck/db";
import {
  EffortSchema,
  MCP_TOOL_NAMES,
...
export async function createPilotDeckMcpServer(opts: {
  db: McpDatabase;
  ctx: AuthContext;
}): Promise<McpServer> {
  ...
  const server = new McpServer(
    { name: "pilotdeck", version: APP_VERSION },
    { instructions: instructionsWithProjects(projects) },
  );

  for (const row of projects) {
    if (!row.context?.trim()) continue;
    const uri = `pilotdeck://project/${row.idPrefix}/context`;
