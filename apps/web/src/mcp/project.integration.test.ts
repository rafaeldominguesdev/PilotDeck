import { project, projectContextAudit, workspace } from "@pilotdeck/db";
...
describe("projects over MCP", () => {
  ...
  it("creates a project and a card in it using only MCP, no database access", async () => {
    ...
    const created = await invokeTool(world.db, ctx(), "project_create", {
      name: "PilotDeck",
      repo_url: "https://github.com/rafaeldominguesdev/pilotdeck",
    });
    expect(created.ok).toBe(true);
    if (!created.ok) return;
    const out = ProjectCreateOutputSchema.parse(created.value);
    expect(out.project.id_prefix).toBe("PD");
    expect(out.project.name).toBe("PilotDeck");
    expect(out.project.repo_url).toBe("https://github.com/rafaeldominguesdev/pilotdeck");
    ...
    expect(list.projects.map((row) => row.id_prefix)).toEqual(["OC", "PD"]);
    ...
    if (!byUuid.ok) return;
    expect(TaskCreateOutputSchema.parse(byUuid.value).task.short_id).toBe("PD-1");
    ...
    expect(second.task.short_id).toBe("PD-2");
  ...
  it("renames a project and shows the new name in project_list", async () => {
    ...
    expect(out.repo_url).toBe("https://github.com/rafaeldominguesdev/pilotdeck");
